...
def get_last_token_attention_map(attn_map,image_name ,filename="last_token_attention_map.png", cmap="Blues"):
    ...
    plt.tight_layout()
    os.makedirs(f"attentions/{image_name}", exist_ok=True)
    plt.savefig(f"attentions/{image_name}/{filename}")
    plt.close()
    ...
    plt.axis("off")
    plt.tight_layout()
    plt.savefig(f"attentions/{image_name}/average_{filename}")
    plt.close()
        ...
        while len(prev) < 10:
            ...
            if save_attention_map:
                for k in cached_tensors.keys():
                    ...
                    get_last_token_attention_map(attn_map,image_name, filename=f"pred{len(prev)}_layer{k}.png")
                    continue
            ...
            for pred, prob in zip(batch_preds, batch_probs):
                ...
                if pred not in prev:
                    ...
                    if pred == "animal":
                        break
                    masked_input = compute_sam_mask(model_lang_sam,pred, img_path, masked_input)
                    print(f"| prob: {prob:.5f} - {pred}")
                    break
